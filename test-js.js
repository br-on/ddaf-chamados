document.addEventListener("DOMContentLoaded", () => {
    ...
    function updateUI(demandasAgrupadas, demandas) {
        ...
        // Mapeamento dos IDs das divs para os tipos de demanda
        const divsDemanda = {
            "Elétrica": "div-img-eletrica",
            "Hidráulica": "div-img-hidraulica",
            "Infraestrutura": "div-img-infraestrutura",
            "Climatização": "div-img-climatizacao",
            "Capinação": "div-img-capinacao",
            "Inservíveis": "div-img-inserviveis",
            "Almoxarifado": "div-img-almoxarifado",
            "Agua-mineral": "div-img-agua-mineral",
            "Carro-pipa": "div-img-carro-pipa",
            "Lixo-infectado": "div-img-infectado",
            "Poda": "div-img-poda",
            "Reparos": "div-img-reparos"
        };
    
        // Inicialmente, todas as divs ficam opacas
        Object.values(divsDemanda).forEach(divId => {
            const div = document.getElementById(divId);
            if (div) {
                div.classList.add("div-opaca");
            }
        });
    
        demandas.forEach(demanda => {

            const tipoDemanda = demanda.tipo_demanda;
            const andamento = demanda.andamento ? demanda.andamento.toLowerCase() : "recebido";
            const divId = divsDemanda[tipoDemanda];

            if (divId) {
                const div = document.getElementById(divId);
                if (div) {
                    div.classList.remove("div-opaca");
                    div.classList.add(`andamento-${andamento}`);
                }
            }
            ...
            if (containerId) {
                ...
                demandaDiv.innerHTML = `
                    <div class="demanda-img ${imgClass}">
